...
#samtools mpileup -u -f ${REF} ${BAM}1_sample.mem.sorted.rmdup.bam | bcftools view -vcg - > variant_sample1.raw.bcf
#2. Trasnformar em VCF
#bcftools view variant_sample1.raw.bcf > 1_sample.variant.vcf

samtools mpileup -uf ${REF} ${BAM}1_sample.mem.sorted.rmdup.bam | bcftools call -vmO v -o 1_sample.vcf --ploidy 1 --threads 2

# Picard Estatística
picard CollectAlignmentSummaryMetrics R=${REF} I=${BAM}1_sample.mem.sorted.rmdup.bam O=statistics.txt
